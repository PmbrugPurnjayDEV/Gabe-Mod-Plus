...
void CHL2MP_Player::Spawn(void)
{
	...
	if ( RandomInt( 0, 1 ) == 0 )
	{
		// Citizen
		SetModel(
			g_ppszRandomCitizenModels[
				RandomInt( 0, ARRAYSIZE( g_ppszRandomCitizenModels ) - 1 )
			]
		);
	}
	else
	{
		// Combine
		SetModel(
			g_ppszRandomCombineModels[
				RandomInt( 0, ARRAYSIZE( g_ppszRandomCombineModels ) - 1 )
			]
		);
	}
...
void CHL2MP_Player::PickupObject( CBaseEntity *pObject, bool bLimitMassAndSize )
{
	...
	// Can't be picked up if NPCs are on it
