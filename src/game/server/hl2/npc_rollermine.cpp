...
void CNPC_RollerMine::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
    ...
    // Stop turning (AI torque target)
    m_RollerController.m_vecAngular = vec3_origin;

    // HARD STOP the actual physics object spin/velocity (THIS is the big one)
    IPhysicsObject *pPhys = VPhysicsGetObject();
    if ( pPhys )
    {
        Vector zeroVel( 0, 0, 0 );
        AngularImpulse zeroAng( 0, 0, 0 );
        pPhys->SetVelocity( &zeroVel, &zeroAng );
        pPhys->Wake();
    }
...
void CNPC_RollerMine::OnPhysGunDrop( CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason )
{
    ...
    // Keep controller OFF during stun; it will be re-enabled by AI once active
    m_RollerController.Off();
    m_RollerController.m_vecAngular = vec3_origin;

    // Optional: also stop any leftover spin on drop
    IPhysicsObject *pPhys = VPhysicsGetObject();
    if ( pPhys )
    {
        AngularImpulse zeroAng( 0, 0, 0 );
        pPhys->AddVelocity( NULL, &zeroAng ); // no-op-ish; safest is SetVelocity, but you may want to preserve throw
    }
