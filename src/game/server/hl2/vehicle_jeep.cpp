...
void CPropJeep::Spawn( void )
{
	...
	m_vecLastServerOrigin = GetAbsOrigin();
	m_angLastServerAngles = GetAbsAngles();
}
...
void CPropJeep::Think( void )
{
    ...
    if ( !m_bInitialHandbrake )	// after initial timer expires, set the handbrake
	{
		...
		if ( m_hPlayer && fabs(m_VehiclePhysics.GetHLSpeed()) > 1.0f )
		{
	...
	// --- Vehicle position smoothing ---
	Vector cur = GetAbsOrigin();
	Vector delta = cur - m_vecLastServerOrigin;

	// Only smooth small corrections
	if ( delta.LengthSqr() < 64.0f )
	{
		SetAbsOrigin( Lerp( 0.15f, m_vecLastServerOrigin, cur ) );
	}
